...
def astar_faire_une_etape(grille, etat, arrivee, couts):
    ...
    # 4) Extraire le meilleur nœud
    fcur, gcur, courant = heapq.heappop(pq)
    frontiere.discard(courant)
...
def reconstruire_chemin(parent, depart, arrivee):
    """
    Reconstruire le chemin depuis arrivee en remontant parent[].

    Comportement attendu :
    - Si arrivee pas dans parent -> None
    - Sinon : remonter jusqu'à None, puis reverse
    - Retourner None si le premier noeud du chemin n'est pas depart
