...
def astar_faire_une_etape(grille, etat, arrivee, couts):
    ...
    # 1) PQ vide : pas de chemin possible
    if not pq:
        etat["termine"] = True
        etat["trouve"] = False
        return

    # 2) Extraire le nœud de f minimal en ignorant les entrées périmées
    while pq:
        f, g_val, courant = heapq.heappop(pq)
        if courant in closed:
            continue
        if g_val != g.get(courant, float("inf")):
            continue
        break
    else:
        etat["termine"] = True
    ...
    for vr, vc, _ in voisins_4(grille, *courant):
        ...
        if g_tentative < g.get(voisin, float("inf")):
            g[voisin] = g_tentative
            parent[voisin] = courant
            f_voisin = g_tentative + heuristique_manhattan(voisin, arrivee)
            heapq.heappush(pq, (f_voisin, g_tentative, voisin))
