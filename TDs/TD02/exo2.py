...
def astar_initialiser(depart, arrivee):
    ...
    """
    h = heuristique_manhattan(depart, arrivee)
    prio = h  # Glouton : priorité = h uniquement
    pq = []
    heapq.heappush(pq, (prio, depart))
    return {
        "pq":       pq,
        "visite":   set(),
        "frontiere": {depart},
...
def astar_faire_une_etape(grille, etat, arrivee, couts):
    ...
    # 2) Purge des entrées périmées + extraction du meilleur nœud
    courant = None
    while pq:
        prio, nxt = heapq.heappop(pq)
        if nxt in visite:
            continue
        # Pour le glouton la priorité est h ; on ignore si g est obsolète
        # (utile si on avait réinséré avec un meilleur g)
        if g.get(nxt, float("inf")) < prio and prio != heuristique_manhattan(nxt, arrivee):
            # entrée obsolète (g amélioré depuis) — on ignore
            continue
        courant = nxt
        break

    # 3) PQ épuisée sans trouver de nœud valide
    if courant is None:
        etat["termine"] = True
        etat["trouve"]  = False
        return

    # 4) Mettre à jour les ensembles et le nœud courant
    visite.add(courant)
