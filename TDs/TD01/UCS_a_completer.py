...
# ============================================================
# MODE DE COÛT (sélectionnable via touche C en jeu)
# ============================================================
# MODE_COUT = 1 : coûts aléatoires [1..9] (seed stable)  ← état initial
# MODE_COUT = 2 : distance en colonnes jusqu'à E          ← heuristique colonne
# MODE_COUT = 3 : distance de Manhattan jusqu'à E         ← heuristique optimale
MODE_COUT_INITIAL = 1
...
def ucs_initialiser(depart):
    ...
    """
    return {
        "pq": [(0, depart)],       # min-heap : (coût_g, nœud)
        "visite": set(),           # closed set : nœuds définitivement traités
        "frontiere": {depart},     # open set  : nœuds en attente dans la PQ
        "parent": {depart: None},  # arbre de remontée pour reconstruire le chemin
        "g": {depart: 0},          # meilleur coût g connu depuis le départ
        "courant": None,           # dernier nœud extrait (affiché en bleu)
        "termine": False,          # True quand l'algo s'arrête
        "trouve": False,           # True si E a été atteint
    }
...
def ucs_faire_une_etape(grille, etat, arrivee, couts):
    ...
    # 2) Purger les entrées périmées (nœud déjà dans closed, ou coût obsolète)
    while etat["pq"]:
        gcur, node = etat["pq"][0]
        if node in etat["visite"]:
            heapq.heappop(etat["pq"])
            continue
        if gcur != etat["g"].get(node):
            heapq.heappop(etat["pq"])
            continue
        break

    # 3) PQ vide après purge = échec
    if not etat["pq"]:
        etat["termine"] = True
        etat["trouve"] = False
        etat["courant"] = None
    ...
    # 4) Extraire le min
    gcur, courant = heapq.heappop(etat["pq"])
    etat["frontiere"].discard(courant)
    etat["courant"] = courant
    etat["visite"].add(courant)
    ...
    for rr, cc, _ in voisins_4(grille, courant[0], courant[1]):
        nxt = (rr, cc)
        if nxt in etat["visite"]:
            continue
        new_g = gcur + cout_case(couts, nxt)
        if new_g < etat["g"].get(nxt, float("inf")):
            etat["g"][nxt] = new_g
            etat["parent"][nxt] = courant
            heapq.heappush(etat["pq"], (new_g, nxt))
            etat["frontiere"].add(nxt)
...
class AppliUCS:
    ...
    def __init__(self, grille):
        ...
        # ------------------------------------------------------------
        # COÛTS PAR CASE — 3 modes (touche C pour basculer)
        # ------------------------------------------------------------
        self.mode_cout = MODE_COUT_INITIAL
        self._rng_couts = random.Random(42)   # RNG stable pour le mode 1
        self.couts = self._generer_couts(self.mode_cout)
    ...
    def _generer_couts(self, mode):
        """
        Génère la carte de coûts d'entrée selon le mode choisi.

        Mode 1 — Aléatoire (seed 42, reproductible) :
            Coût ∈ [1..9] tiré au hasard. S et E valent 1.
            UCS explore "au hasard" selon ces poids.

        Mode 2 — Distance en colonnes jusqu'à E :
            Coût = |c - c_E| + 1  (on ajoute 1 pour éviter le coût 0).
            Les cases proches de la colonne de E sont "moins chères".
            UCS est guidé horizontalement vers E.

        Mode 3 — Distance de Manhattan jusqu'à E :
            Coût = |r - r_E| + |c - c_E| + 1
            C'est une heuristique admissible et cohérente.
            UCS se comporte comme A* : il explore en priorité les cases
            les plus proches de E, ce qui réduit fortement le nombre de
            nœuds explorés.
        ...
        Returns:
            dict[tuple[int,int], int]: coûts par case.
        """
        couts = {}
        r_e, c_e = self.sortie
        rng = random.Random(42)   # seed fixe → reproductible à chaque appel

        for r in range(self.lignes):
            for c in range(self.colonnes):
                if not est_traversable(self.grille, r, c):
                    continue
                pos = (r, c)
                ch = self.grille[r][c]

                if mode == 1:
                    # Coûts aléatoires [1..9] ; S et E valent 1
                    if ch in ("S", "E"):
                        couts[pos] = 1
                    else:
                        couts[pos] = rng.randint(1, 9)

                elif mode == 2:
                    # Distance en colonnes jusqu'à E (+ 1 pour éviter coût nul)
                    couts[pos] = abs(c - c_e) + 1

                elif mode == 3:
                    # Distance de Manhattan jusqu'à E (+ 1 pour éviter coût nul)
                    couts[pos] = abs(r - r_e) + abs(c - c_e) + 1
    ...
    def dessiner_barre_bas(self):
        ...
        noms_modes = {1: "1-Aléatoire", 2: "2-Colonnes", 3: "3-Manhattan"}
        self._dessiner_texte(12, y + 54, f"Mode coût : {noms_modes[self.mode_cout]}  (C pour changer)", self.font_petit, COL_TEXTE_MUET)

        self._dessiner_texte(self.largeur_fenetre - 520, y + 8, "Commandes :", self.font_petit, COL_TEXTE_MUET)
        self._dessiner_texte(
            self.largeur_fenetre - 900, y + 32,
            "E=UCS Auto   ESPACE=Pas à Pas   P=Chemin Opt   R=Reset   F=Brouillard   C=Mode coût   Q=Quitter",
            self.font_petit
        )
    ...
    def run(self):
        ...
        Commandes:
            ...
            R: reset
            F: brouillard on/off
            C: changer le mode de coût (1=aléatoire, 2=colonnes, 3=Manhattan)
        ...
        while True:
            ...
            for ev in pygame.event.get():
                ...
                if ev.type == pygame.KEYDOWN:
                    ...
                    # C = changer le mode de coût (cycle 1 → 2 → 3 → 1)
                    if ev.key == pygame.K_c:
                        self.mode_cout = (self.mode_cout % 3) + 1
                        self.couts = self._generer_couts(self.mode_cout)
                        self.reinitialiser_tout()
